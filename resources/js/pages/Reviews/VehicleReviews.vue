<template>
  <AppLayout :title="`Évaluations - ${vehicle.brand} ${vehicle.model}`">
    <template #header>
    ...
    <div class="py-12">
      <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="flex items-start justify-between">
              ...
              <div class="text-center">
                <div class="flex items-center mb-2">
                  <StarRating :rating="vehicle.rating" :size="'lg'" />
                </div>
                <p class="text-sm text-gray-600 dark:text-gray-400">
                  {{ vehicle.rating_count }} évaluation{{ vehicle.rating_count > 1 ? 's' : '' }}
                </p>
              </div>
            </div>
          </div>
        ...
        <div v-if="reviews.data.length === 0" class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg p-6 text-center">
          <p class="text-gray-500 dark:text-gray-400">Aucune évaluation pour ce véhicule.</p>
        </div>
